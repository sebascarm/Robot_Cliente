        ...
        for thread in ThreadAdmin.threads:
            if thread.state:
                if not thread.thread.is_alive():
                    mensaje = ("THREAD ID: " + str(thread.ident) + " " + str(
                        thread.name) + " FINISHED - TOTAL THREADS: " + str(thread.total_threads()))
...
class ThreadAdmin(object):
    ...
    def start(self, process, argument='', name='', time_to_kill=3, callback='', enviar_ejecucion=False):
        ...
        if not self.state:
            ...
            mensaje = ("START THREAD ID: " + str(self.ident) + " " + str(self.name) + " - TOTAL THREADS: " + str(
                self.total_threads()))
